<template>
  <div class="login-page">
    <div class="form">
      <form v-if="mode === 'register'" class="register-form stack" @keyup.enter="signup()">
        <h2>👋 Register Here</h2>
      ...
      <form v-if="mode === 'login'" class="login-form stack" @keyup.enter="login()">
        <h2>🔐 Login Here</h2>
        <label for="email">Email</label>
        <input id="email" v-model="crendentials.email" type="text" placeholder="hey@email.com" />
        ...
        <input v-model="crendentials.password" :type="passwordType" placeholder="******" />
...
export default {
  ...
  methods: {
    ...
    toggleMode() {
      this.mode === 'register' ? (this.mode = 'login') : (this.mode = 'register');
    },
    signup() {
