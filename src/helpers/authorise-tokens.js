...
function detectExternalAccessToken() {
  try {
    ...
    console.error('Something went wrong trying to extract the access token', error);
    return null;
  }
}
...
function detectRecoveryToken() {
  try {
    // split the hash where it detects `confirmation_token=`. The string which proceeds is the part which we want.
    const token = decodeURIComponent(document.location.hash).split('recovery_token=')[1];
    return token;
  } catch (error) {
